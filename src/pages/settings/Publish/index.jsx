...
const Publish = () => {
	...
	const handlePublish = async () => {
		try {
			const res = await publishDocument(currUser.username, state.id, state.file, state.desc);
			if (res === "success")
				setAlert({
					type: "success",
	...
	return (
		<SettingsLayout>
			<div className="p-2 space-y-6">
				{/* Url. */}
				<div className="flex items-center justify-between px-3 py-2 font-semibold underline border rounded-lg shadow-md border-dark/50 text-dark/50">
					...
					<Button
						...
						onClick={(e) => {
							...
							navigator.clipboard.writeText(
								"https://schedule-maker.netlify.app/publish/" + currUser.username,
							);
						}}
				...
				{state.file ? (
					<div className="space-y-3">
						<Input
							...
							onChange={(e) => dispatch({ type: "ID", payload: { value: e.target.value } })}
						/>
						<Input
							type="textarea"
							label="description"
				...
				<div className="">
					<div className="flex items-center pr-1 border-b-2 border-dark/50 bg-primary text-start text-light">
						<div className="grid w-full grid-cols-12 p-2 font-semibold">
							...
							<div className="hidden col-span-3 text-center md:block">Created At</div>
						</div>
					...
					{documents.map((value, docIndex) => (
						<div
							key={value.id}
							className={`menu-item group flex ${docIndex % 2 === 0 && "bg-dark/10"}`}>
							<div onClick={(e) => handleDetail(value)} className="grid w-full grid-cols-12">
							...
							<MoreMenu
								...
								options={[
									...
									["delete", () => setModel({ type: "delpubdoc", id: value.id, documents }), IcBin],
								]}
							/>
						</div>
					))}
